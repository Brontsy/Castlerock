		...
		public class PocoData
		{
			...
			private static Func<object, object> GetConverter(bool forceDateTimesToUtc, PocoColumn pc, Type srcType, Type dstType)
			{
				...
				if (converter == null)
				{
                    ...
                    else if (dstType.IsEnum && (srcType == typeof(string) || srcType == typeof(int)))
                    {
                        converter = delegate(object src) { return (src.ToString().Length == 1 ? Enum.ToObject(dstType, src.ToString()[0]) : Enum.Parse(dstType, src.ToString())); };
                    }
